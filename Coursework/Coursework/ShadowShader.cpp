...
void ShadowShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, ShadowMap* shadowMap[19], Light* light[4], XMFLOAT4 lightAttenuation[4])
{
	...
	// directional light and spotlight matrix view and projection
	for (int i = 0; i < 2; i++)
	{
		// Transpose the matrices to prepare them for the shader.
	...
	//dataPtr->lightProjection[2] = light[1]->getProjectionMatrix();
